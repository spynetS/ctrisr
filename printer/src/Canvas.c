...
void setBorder(Canvas *canvas, int borderWith){
    ...
    //add x and y so our border fits
    canvas->x++;
    canvas->y++;
    ...
    // horzontal lines
    for(int i = 0; i < canvas->width*2+1; i +=2){
        setCharAt(i+canvas->x-1,canvas->y-1,top);
        setCharAt(i+canvas->x-1,canvas->height+canvas->y,bottom);
    }
    //corsers
    setCharAt(canvas->x-1,canvas->y-1,joinTopRight);
    setCharAt(0+canvas->x-1,canvas->height+canvas->y,joinBottomRight);
    setCharAt(canvas->width*2+canvas->x,canvas->y-1,joinTopLeft);
